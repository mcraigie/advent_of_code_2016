class Screen
	...
	def reset() # only used in testing
		@state = Array.new(@state.length) { |e| Array.new(@state[0].length) { |e| 0 }}
	end

	def render() # only used in testing
		p '-' * ((@state[0].length * 2) - 1)
		@state.each do |row|
			p row.map {|element| "#{element} "}.join.strip
		end
	end
	...
	def detect_voltage()
		@state.map {|row| row.count(1)}.reduce(0, :+)
	end
end
